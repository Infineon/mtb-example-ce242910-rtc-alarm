# PSOC&trade; Control C1 MCU: RTC alarm

This code example demonstrates a real-time clock (RTC) alarm interrupt configured to trigger after a minute to periodically blink an LED on the [PSOC&trade; Control C1 MCU evaluation kits](#supported-kits-make-variable-target).

[View this README on GitHub.](https://github.com/Infineon/mtb-example-ce242910-rtc-alarm)

[Provide feedback on this code example.](https://yourvoice.infineon.com/jfe/form/SV_1NTns53sK2yiljn?Q_EED=)
...
The following example clones the "[RTC alarm](https://github.com/Infineon/mtb-example-ce242910-rtc-alarm)" application with the desired name "RTC alarm" configured for the *KIT_PSC1M3_EVK* BSP into the specified working directory, *C:/mtb_projects*:

   ```
   project-creator-cli --board-id KIT_PSC1M3_EVK --app-id mtb-example-ce242910-rtc-alarm --user-app-name RTC alarm --target-dir "C:/mtb_projects"
   ```
...
4. Confirm that the kit LED blinks at approximately 1 Hz because of the SysTick interrupt. The RTC is configured to generate an alarm after 1 minute

5. Confirm that the kit LED stops blinking and remains glowing after a minute

> **Note:** To reset the RTC and to test the CE again after the initial programming, disconnect the cable for a period of time and then reconnect it.
...
- **Step 3:** Involves starting the RTC module using the `Cy_RTC_Start()` function and entering an endless `while` loop. In the `while` loop, as soon as the RTC alarm event occurs, the precondition (`timer_interrupt_flag = TRUE`) is fulfilled to set the output of the LED to HIGH. Afterward, the LED remains glowing.
...
Document title: *CE242910* – *PSOC&trade; Control C1 MCU: RTC alarm*
